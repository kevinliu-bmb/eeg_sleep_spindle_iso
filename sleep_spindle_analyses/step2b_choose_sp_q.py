...
def main():
    ...
    ch_names = ['EEG_1', 'EEG_2', 'EEG_3', 'EEG_4', 'EEG_5', 'EEG_6', 'EEG_7', 'EEG_8', 'EEG_9', 'EEG_10', 'EEG_11', 'EEG_12', 'EEG_13', 'EEG_14', 'EEG_15', 'EEG_16', 'EEG_17', 'EEG_19', 'EEG_20', 'EEG_21', 'EEG_22', 'EEG_23', 'EEG_24', 'EEG_25', 'EEG_26', 'EEG_27', 'EEG_28', 'EEG_31', 'EEG_32']
    ch_names = [x.replace('_',' ') for x in ch_names]
    ...
    for i in tqdm(range(len(df))):
        ...
        Fs = edf.info['sfreq']
        assert np.in1d(ch_names, edf.ch_names).all()
        ...
        ids2 = np.random.choice(np.arange(len(df_sp)), min(len(df_sp), 100), replace=False)
        for j in tqdm(ids2):
            q = df_sp.Q.iloc[j]
            save_path = os.path.join(sp_vis_dir, f'sp_vis_q={q:.1f}_{sid}_{j}.png')
    ...
    q = df.Q.values
    plot_perf(y, q, save_path='spindle_q_perf.pdf')
